...
func runDemo() {
    do {
        ...
        let dto = try decoder.decode(StudyPlanDTO.self, from: Data(samplePlanJSON.utf8))

        printDivider("第 1 步：远程 DTO")
        printDTO(dto)
        ...
        try store1.replaceStoredPlan(with: dto)
        let recordsAfterSave = try store1.fetchStoredPlans()

        printDivider("第 2 步：映射并保存成 SwiftData Record")
        ...
        printDivider("第 3 步：重建容器后再次读取 Record")
        printRecords(persistedRecords)

        let domainPlans = try store2.fetchDomainPlans()
        printDivider("第 4 步：映射成领域模型")
