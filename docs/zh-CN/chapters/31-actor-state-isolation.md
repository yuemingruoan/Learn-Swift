...
如果你想把这一章的内容真正落实到一个“有共享状态冲突，但功能已经齐全”的项目里，可以继续完成下面这道重构作业：

- 作业答案：`exercises/zh-CN/answers/31-actor-state-isolation.md`
- 起始工程：`exercises/zh-CN/projects/31-actor-state-isolation-starter`
- 参考答案工程：`exercises/zh-CN/answers/31-actor-state-isolation`

starter project 当前已经能跑出两类典型冲突：

- `StudyProgressStore` 的完成数更新会丢失。
- `WorkshopCenter` 的名额检查和扣减会被并发破坏。
