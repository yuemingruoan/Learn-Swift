...
- 建议下一章：35. 网络请求入门：把远程 JSON 带回 Swift（待补充）
- 下一章：35. 网络请求入门：把远程 JSON 带回 Swift（待补充）
...
- 理解为什么 JSON 会频繁出现在接口、配置和数据交换场景里
...
- 真正的网络请求发起、响应检查和远程数据加载，放到下一章单独展开。

## 为什么先单独学习 JSON

很多初学者第一次接触接口数据时，会把两件事混在一起：
...
因为如果你连返回内容的结构都还看不清，那么后面即使学会了请求函数，也很容易停在一种模糊状态：
...
也就是说，这一章先学 JSON，并不是只为了后面的网络请求，而是因为：
...
- 35. 网络请求入门：把远程 JSON 带回 Swift（待补充）
