...
本章内容以“控制台 + 最小网络层”为主，不掺 UI，也不把问题一下子扩成“通用网络框架”。
...
第 38 章解决的是“把一个请求发出去”。到这里，问题不再是能不能发，而是接口一多之后，代码还能不能继续收得住。
...
这一章要做的，就是用一套尽量克制的分层把这些重复和混乱收起来。

先看本章最后要落到的结构图，不用急着把细节一次记全：
...
但也有一些东西，这个阶段先别急着抽太深，不然“最小网络层”很快就会写成一个小框架：

- 自动重试、退避（backoff）
- 请求拦截器、插件系统、中间件链
- 通用缓存策略、离线策略

本章只做够用的抽取，能支撑十几个接口继续往下写就行。
...
把这些差异收进一个类型里，就是 `Endpoint`。它只做两件事：
...
到这里，请求还没发出去，但“每个接口到底哪里不一样”已经有了固定落点。
...
所以，“从 Endpoint 构造 URLRequest” 这一步应该是 `throws`，而且要能看出错误发生在构造阶段。

先把本章要用的错误模型摆出来，后面再逐条看：
...
- 这一层要把“URL 构造失败”和“请求体编码失败”明确抛出来，因为它们通常是**代码错误/配置错误**，不是网络波动。
