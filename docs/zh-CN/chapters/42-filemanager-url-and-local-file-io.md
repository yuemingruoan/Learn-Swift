...
本章聚焦**本地文件 I/O**：路径、目录、文件存在性、以及读写失败怎么处理。
...
这些问题的共同点是：它们都绕不开**本地文件系统**。本章先把文件系统基础讲清楚，再进入下一章的“模型持久化/本地缓存”。
...
实际上 `URL` 只是一个统一的“资源定位符”，它可以定位很多东西，其中最常见的是：
...
例如在当前的 macOS 开发环境里、在命令行工具里、在沙盒 App 里，同一个 `.documentDirectory` 背后返回的具体路径都可能不同。教程里更关心的是：

- 你知道这个枚举值代表哪一类目录
- 然后通过 `FileManager` 向系统拿到当前环境下真正可用的 `URL`

也就是说，`FileManager` 不是让你自己写字符串去猜路径，而是让你先说清楚：
...
不过对当前阶段来说，你不需要把 `SearchPathDirectory` 的所有 case 都背下来。大多数目录有“平台历史包袱”或更偏桌面系统语义，例如：
