...
- 文稿：`docs/zh-CN/chapters/42-filemanager-url-and-local-file-io.md`
- 示例项目：`demos/projects/42-filemanager-url-and-local-file-io`
...
1. 先把目录语义读清楚
- Documents、Caches、tmp 的区别比记 API 名字更重要

2. 再看 URL 和 path 的关系
- 先形成“URL 描述位置、path 只是字符串形式”的心智模型

3. 最后再看 String / Data / JSON 文本这些落盘形态
- 你后面做快照缓存时，会直接复用这里的判断
...
- 把文件 URL 当成普通字符串路径来手工拼接
- 不区分 Documents / Caches / tmp，结果把缓存文件放进用户文档目录
- 文本内容也一律先转成 `Data`，或者二进制内容先绕成 `String`

排错顺序建议固定成这样：

1. 先看目标目录选得对不对
2. 再看 URL 是不是通过系统目录 + `appendingPathComponent` 生成
3. 再看文件是否存在
4. 最后区分“文件不存在”和“文件存在但读取失败”

Demo 里应该重点观察：

- 标准目录 URL 的真实输出
- 创建目录和检查存在性这两步是如何配合的
- `String`、`Data`、JSON 文本三种不同读写对象的选择理由
- 缺失文件分支和“路径 / URL / 对象类型”这三个概念如何重新对齐
