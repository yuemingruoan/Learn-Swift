...
- 文稿：`docs/zh-CN/chapters/45-swiftdata-advanced-query-sort-relationships-and-boundaries.md`
- 示例项目：`demos/projects/45-swiftdata-advanced-query-sort-relationships-and-boundaries`
...
SwiftData 很适合处理结构化条件和稳定排序，但它不是`runtime`，也不是你的全部业务逻辑。
...
- 先把全部数据读出来，再在业务层做本来应该由查询表达的筛选
- 只想着“能删掉就行”，却不先定义删掉父对象后的产品语义
- 把排序规则写成临时 if/else，结果每个调用点都不一致
...
2. 再确认排序字段和顺序是否稳定
3. 再看关系是否正确建立
4. 最后单独验证 deleteRule 触发后的结果是否符合产品语义
