...
本章以“控制台 + 最小业务链路”为主，尽量减少 UI 对语言与设计讨论的干扰。
...
第 21 章你已经学过协议语法，第 24 章你也见过泛型如何复用逻辑。那为什么到了第 46 章，我们还要再谈一次“协议”？

原因很简单：**前几章的内容开始出现真实工程的三类依赖**：
...
本章不是协议语法复习，也不是要你背“架构模式大全”。它只做一件事：在你已经经历了“快照缓存 -> 本地记录 CRUD -> 结构化读取与关系规则”之后，用协议和依赖注入把这些可替换的依赖从业务逻辑里剥离出来，让你写出**更可测、更可维护**的代码。
...
更直接的总结是：**当依赖被写死时，业务逻辑就被迫跟着真实环境跑**。而测试需要的是“可控的环境”和“可观察的行为”。
...
协议怎么抽象，最容易踩的坑是“想写一个万能协议”。本章刻意强调相反方向：**协议越小越好，职责越清楚越好**。
...
你可以把这一步理解为：**把不可控的世界（网络/磁盘/数据库）“包起来”，让业务只面对一个可控的接口**。

### 模块 3：依赖注入

有了协议，还差最后一步：让业务代码拿到“实现”，但不把实现写死。这就是依赖注入（Dependency Injection, DI）。

依赖注入听起来很大，但你可以先把它理解成一句话：
...
替身实现的价值在于“可控”和“可观察”。常见有三种（名字不重要，关键是目的）：
...
到这里，你应该能看到“替换实现”的真正含义：`ArticleService` 不用改一行代码，就能通过传入不同依赖在不同环境运行。

### 模块 5：可测试设计（把可替换落到可验证）

本章想落地的可测试性不是“把 XCTest 教一遍”，而是让你看到一个更关键的因果关系：
